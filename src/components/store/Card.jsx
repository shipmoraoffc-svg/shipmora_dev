...
export default function Card({ p, setCart, wl, setWl, user, onLogin }) {
  ...
  return (
    ...
    >
      <div style={{ height: 170, position: "relative", overflow: "hidden", background: "#f3f4f6" }}>{p.image && !imgError ? (<img src={p.image} alt={p.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={() => setImgError(true)} />) : (<div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 68, background: `linear-gradient(135deg,${p.color}22,${p.color}44)` }}>{p.emoji}</div>)}
